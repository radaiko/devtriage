...
| OQ-22a | **Auth mechanism** — passkeys (WebAuthn) vs social login (Sign in with GitHub/Google/Apple) vs email+password / magic-link? | Prefer the least PII and no stored passwords. Passkeys store only public keys; social login reuses an existing identity (GitHub already in the picture); email needs recovery/sending infra. Mind NFR-DEP for any server-side library. |
