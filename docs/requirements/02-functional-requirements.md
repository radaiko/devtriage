...
| FR-EXTRACT-2 | 🔴 | At minimum, Markdown checkbox lines (`- [ ]`) are recognized as action items (v1 explicit detection, OQ-13). |
| FR-EXTRACT-2a | 🟠 | v1 explicit detection also recognizes inline text markers developers commonly use (`TODO:`, `FIXME:`, `@todo`). These are high-confidence → auto-created (OQ-12). Detected with trivial in-house parsing (no deps). |
...
| FR-EXTRACT-5 | 🟠 | Heuristic detection of imperative/action sentences beyond explicit markers — **lightweight, rule-based, in-house** (imperative-verb-initial lines, cue phrases); **no heavy NLP/ML dependency** (NFR-DEP). Fuzzy → suggest-and-confirm (OQ-13). |
